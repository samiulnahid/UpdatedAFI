  ...
  return gulp.src(solution)
      .pipe(msbuild({
          ...
          properties: {
            ...
            DeployDefaultTarget: "WebPublish",
            WebPublishMethod: "FileSystem",
            publishUrl: path.join(__dirname, ".dist"),
            // EnableUpdateable: false,
            // PrecompileBeforePublish: true,
            // UseMerge: "true",
            // ContentAssemblyName: "Aha.Sitecore.CompiledViews",
            // WDPMergeOption: "MergeAllPagesAndControlToASingleAssembly"
          }
        }));
});
...
gulp.task("Publish-Assemblies", function () {
  var root = "./src";
  var binFiles = root + "/**/code/**/bin/Sitecore.{Feature,Foundation,Aha}.*.{dll,pdb}";
...
gulp.task("Auto-Publish-Assemblies", function () {
  ...
  var files = "/**/Sitecore.{Feature,Foundation,Aha}.*.{dll,pdb}";;
  var destination = config.websiteRoot + "/bin/";
  gulp.src(roots, { base: root }).pipe(
    foreach(function (stream, rootFolder) {
      gulp.watch(rootFolder.path + files, function (event) {

...
{
    public class LeadGenerationFormSaveHandler : IFormSaveHandler<LeadGenerationForm>
    {
        ...
        public LeadGenerationFormSaveHandler(ISessionRepository sessionRepository, ILeadGenerationFormRepository leadGenerationRepository, IFormEmailHandler emailHandler, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public void ApplySessionFieldChanges(LeadGenerationForm form, string key)
        {
            IEnumerable<KeyValuePair<string, string>> mapper = _leadGenerationRepository.RetrieveFormByKey(key);
            Item formItem = _apiRepository.GetLeadGenerationForm();
            if (mapper.Any())
            {
        ...
        public void ApplySessionFieldChanges(LeadGenerationForm form)
        {
            var mapper = FormExtensions.ApplyZipCodeLookup(_sessionRepository, _zipLookupService, CoverageTypes.LeadGeneration);
            form.ApplyFieldChanges(mapper);
        }
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            LeadGenerationFormSaveModel form = FormExtensions.MapJson<LeadGenerationFormSaveModel>(values.ToString());
        ...
        private void HandleSubmit(LeadGenerationFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            CommonFormSaveResponseModel response = _leadGenerationRepository.SubmitForm(form);
            //INFO: Calling AFI WS
            ...
            //        _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.LeadGeneration);
            //    }
            //    catch (Exception ex)
            //    {
            ...
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    Sitecore.Diagnostics.Log.Info($"LeadGeneration Forms WS Endpoint: {_afiWebServicesSaveHandler.GetCurrentEndpoint()}", typeof(LeadGenerationFormSaveHandler));
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.LeadGeneration, form);
                    string customerServicePhone = _leadGenerationRepository.GetFormCustomerServicePhone();
                    toSave.Add(new KeyValuePair<string, string>("customerServicePhone", customerServicePhone));
                    _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.LeadGeneration);
                    sw.Stop();
                    Sitecore.Diagnostics.Log.Info("STOPWATCH LeadGeneration submit " + sw.Elapsed, "stopwatch");
                }
                catch (Exception ex)
                {
                    sw.Stop();
                    Sitecore.Diagnostics.Log.Info("STOPWATCH: LeadGeneration submit error " + sw.Elapsed, "stopwatch");
                    Sitecore.Diagnostics.Log.Error("Quote Forms error - WS Call Error", ex, Constants.QUOTE_FORMS_LOGGER_NAME);
                }
            }
        ...
        private void HandleAbandon(LeadGenerationFormSaveModel model)
        {
            _leadGenerationRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(LeadGenerationFormSaveModel model)
        {
            CommonFormSaveResponseModel saveResponse = _leadGenerationRepository.SaveForm(model);
            return saveResponse;
        }

        private void HandleSaveAndExit(LeadGenerationFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            string formUrl = _leadGenerationRepository.GetFormUrl();
            toSave.Add(new KeyValuePair<string, string>("formurl", formUrl));
            _emailHandler.HandleSave(toSave, emailRecipient, CoverageTypes.LeadGeneration);
        }

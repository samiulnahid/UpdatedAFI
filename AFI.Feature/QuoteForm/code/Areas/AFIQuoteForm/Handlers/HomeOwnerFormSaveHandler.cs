...
{
    public class HomeOwnerFormSaveHandler : IFormSaveHandler<HomeownerForm>
    {
        private readonly ISessionRepository _sessionRepository;
        private readonly IFormEmailHandler _emailHandler;
        private readonly IHomeownerQuoteFormRepository _formRepository;
        ...
        public HomeOwnerFormSaveHandler(ISessionRepository sessionRepository, IFormEmailHandler emailHandler, IHomeownerQuoteFormRepository formRepository, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            HomeownerFormSaveModel form = FormExtensions.MapJson<HomeownerFormSaveModel>(values.ToString());
        ...
        private void HandleSubmit(HomeownerFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    ...
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.Homeowner, form);
                }
                catch (Exception ex)
                {
            ...
            _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.Homeowner);
        }

        private void HandleAbandon(HomeownerFormSaveModel model)
        {
            _formRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(HomeownerFormSaveModel model)
        {
        ...
        private void HandleSaveAndExit(HomeownerFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            _emailHandler.HandleSave(toSave, emailRecipient, CoverageTypes.Homeowner);
        }

        public void ApplySessionFieldChanges(HomeownerForm form, string key)
        {
        ...
        public void ApplySessionFieldChanges(HomeownerForm form)
        {

...
{
    public class BusinessFormSaveHandler : IFormSaveHandler<BusinessForm>
    {
        ...
        public BusinessFormSaveHandler(ISessionRepository sessionRepository, IBusinessQuoteFormRepository businessRepository, IFormEmailHandler emailHandler, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public void ApplySessionFieldChanges(BusinessForm form, string key)
        {
            IEnumerable<KeyValuePair<string, string>> mapper = _businessRepository.RetrieveFormByKey(key);
            Item formItem = _apiRepository.GetBusinessForm();
            if (mapper.Any())
            {
        ...
        public void ApplySessionFieldChanges(BusinessForm form)
        {
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            BusinessFormSaveModel form = FormExtensions.MapJson<BusinessFormSaveModel>(values.ToString());
        ...
        private CommonFormSaveResponseModel HandleSubmit(BusinessFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            CommonFormSaveResponseModel response = _businessRepository.SubmitForm(form);
            //INFO: Calling AFI WS
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    ...
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.Business, form);
                    string customerServicePhone = _businessRepository.GetFormCustomerServicePhone();
                    toSave.Add(new KeyValuePair<string, string>("customerServicePhone", customerServicePhone));
                    _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.Business_Email_Display);
                }
                catch (Exception ex)
                {
        ...
        private void HandleAbandon(BusinessFormSaveModel model)
        {
            _businessRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(BusinessFormSaveModel model)
        {
            CommonFormSaveResponseModel saveResponse = _businessRepository.SaveForm(model);
            return saveResponse;
        }

        private void HandleSaveAndExit(BusinessFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            string formUrl = _businessRepository.GetFormUrl();

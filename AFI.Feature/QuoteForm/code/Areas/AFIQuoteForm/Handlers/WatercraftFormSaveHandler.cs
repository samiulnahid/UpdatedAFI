...
{
    public class WatercraftFormSaveHandler : IFormSaveHandler<WatercraftForm>
    {
        ...
        public WatercraftFormSaveHandler(ISessionRepository sessionRepository, IFormEmailHandler emailHandler, IWatercraftQuoteFormRepository watercraftRepository, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            WatercraftFormSaveModel form = FormExtensions.MapJson<WatercraftFormSaveModel>(obj["values"].ToString());
        ...
        private CommonFormSaveResponseModel HandleSubmit(WatercraftFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            CommonFormSaveResponseModel response = _watercraftRepository.SubmitForm(form);
            //INFO: Calling AFI WS
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    ...
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.Watercraft, form);
                    string customerServicePhone = _watercraftRepository.GetFormCustomerServicePhone();
                    toSave.Add(new KeyValuePair<string, string>("customerServicePhone", customerServicePhone));
                    _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.Watercraft);
                }
                catch (Exception ex)
                {
        ...
        private void HandleAbandon(WatercraftFormSaveModel model)
        {
            _watercraftRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(WatercraftFormSaveModel model)
        {
            CommonFormSaveResponseModel saveResponse = _watercraftRepository.SaveForm(model);
            ...
            {
                Sitecore.Diagnostics.Log.Error("Quote Forms error - WS Call Error", ex, typeof(WatercraftFormSaveHandler));
            }

            return saveResponse;
        }

        private void HandleSaveAndExit(WatercraftFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            string formUrl = _watercraftRepository.GetFormUrl();
        ...
        public void ApplySessionFieldChanges(WatercraftForm form, string key)
        {
            List<KeyValuePair<string, string>> mapper = _watercraftRepository.RetrieveFormByKey(key);
            Item formItem = _apiRepository.GetWatercraftForm();
            if (mapper.Any())
            {
        ...
        public void ApplySessionFieldChanges(WatercraftForm form)
        {
            var mapper = FormExtensions.ApplyZipCodeLookup(_sessionRepository, _zipLookupService, CoverageTypes.Watercraft);
            form.ApplyFieldChanges(mapper);
        }
    }
}
...
{
    public class MotorhomeFormSaveHandler : IFormSaveHandler<MotorhomeForm>
    {
        private readonly ISessionRepository _sessionRepository;
        private readonly IFormEmailHandler _emailHandler;
        private readonly IMotorhomeQuoteFormRepository _formRepository;
        ...
        public MotorhomeFormSaveHandler(ISessionRepository sessionRepository, IFormEmailHandler emailHandler, IMotorhomeQuoteFormRepository formRepository, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            MotorhomeFormSaveModel form = FormExtensions.MapJson<MotorhomeFormSaveModel>(values.ToString());
        ...
        private CommonFormSaveResponseModel HandleSubmit(MotorhomeFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    ...
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.Motorhome, form);
                    ...
                    _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.Motorhome_Email_Display);
                }
                catch (Exception ex)
                {
        ...
        private void HandleAbandon(MotorhomeFormSaveModel model)
        {
            _formRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(MotorhomeFormSaveModel model)
        {
        ...
        private void HandleSaveAndExit(MotorhomeFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            _emailHandler.HandleSave(toSave, emailRecipient, CoverageTypes.Motorhome_Email_Display);
        }

        public void ApplySessionFieldChanges(MotorhomeForm form, string key)
        {
        ...
        public void ApplySessionFieldChanges(MotorhomeForm form)
        {
            var mapper = FormExtensions.ApplyZipCodeLookup(_sessionRepository, _zipLookupService, CoverageTypes.Motorhome);
            form.ApplyFieldChanges(mapper);
        }

...
{
    public class MobileHomeFormSaveHandler : IFormSaveHandler<MobilehomeForm>
    {
        private readonly ISessionRepository _sessionRepository;
        private readonly IFormEmailHandler _emailHandler;
        private readonly IMobilehomeQuoteFormRepository _formRepository;
        ...
        public MobileHomeFormSaveHandler(ISessionRepository sessionRepository, IFormEmailHandler emailHandler, IMobilehomeQuoteFormRepository formRepository, ICommonRepository commonRepository, IAPIRepository apiRepository, IZipLookupService zipLookupService, IAFIWebServicesSaveHandler afiWebServicesSaveHandler)
        {
        ...
        public CommonFormSaveResponseModel HandlePost(string json, string sessionId)
        {
            ...
            MobilehomeFormSaveModel form = FormExtensions.MapJson<MobilehomeFormSaveModel>(values.ToString());
        ...
        private CommonFormSaveResponseModel HandleSubmit(MobilehomeFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            if (response.QuoteId > 0)
            {
                ...
                try
                {
                    ...
                    _afiWebServicesSaveHandler.ProcessSubmitInformationToAFI(CoverageTypes.Mobilehome, form);
                }
                catch (Exception ex)
                {
            ...
            try
            {
                _emailHandler.HandleSubmit(toSave, emailRecipient, CoverageTypes.Mobilehome_Email_Display);
        ...
        private void HandleAbandon(MobilehomeFormSaveModel model)
        {
            _formRepository.SaveForm(model);
        }

        private CommonFormSaveResponseModel HandleSave(MobilehomeFormSaveModel model)
        {
        ...
        private void HandleSaveAndExit(MobilehomeFormSaveModel form, List<KeyValuePair<string, string>> toSave, string emailRecipient)
        {
            ...
            _emailHandler.HandleSave(toSave, emailRecipient, CoverageTypes.Mobilehome_Email_Display);
        }

        public void ApplySessionFieldChanges(MobilehomeForm form, string key)
        {
        ...
        public void ApplySessionFieldChanges(MobilehomeForm form)
        {
            var mapper = FormExtensions.ApplyZipCodeLookup(_sessionRepository, _zipLookupService, CoverageTypes.Mobilehome);
            form.ApplyFieldChanges(mapper);
        }
    }
}
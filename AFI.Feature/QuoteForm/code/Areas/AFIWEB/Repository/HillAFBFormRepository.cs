...
{
    public interface IHillAFBFormRepository
    {
        int InsertForm(CorviasModel form);
    }
    public class HillAFBFormRepository : IHillAFBFormRepository
    {
        private readonly IDbConnectionProvider _dbConnectionProvider;

        public HillAFBFormRepository(IDbConnectionProvider dbConnectionProvider)
        {
        ...
        public int InsertForm(CorviasModel form)
        {
            ...
            quote.CoverageType = CoverageTypes.Hill_AFB;
        ...
        public int CreateCorvias(CorviasModel entity)
        {
            ...
            using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
            {
                ...
                using (var transaction = db.BeginTransaction())
                {
                    try
                    {
                        var sql = @"insert into dbo.[QuoteHillAFBForm] ([QuoteKey], [CoverageType], [CoverageMonth], [CoverageYear], [AdditionalQuestions], [PaymentMethod], [AgentCallDate], [AgentCallTime], [NameOnCard], [CardNumber], [ExpiryDate], [SecurityCode], [Routing], [AccountNumber])

...
{
    public interface IUCFormRepository
    {
        int InsertForm(CorviasModel form);
    }
    public class UCFormRepository : IUCFormRepository
    {
        private readonly IDbConnectionProvider _dbConnectionProvider;

        public UCFormRepository(IDbConnectionProvider dbConnectionProvider)
        {
        ...
        public int InsertForm(CorviasModel form)
        {
            ...
            quote.CoverageType = CoverageTypes.UC;
            ...
            if (quote.Key > 0)
            {
                ...
                quoteContact.SpouseBirthDate = form.SpouseBirthDate;
                quoteContact.SpouseSSN = form.SpouseSSN;
                quoteContact.SpouseFirstName = form.SpouseFirstName;
                quoteContact.SpouseLastName = form.SpouseLastName;
                quoteContact.SpouseSuffix = form.SpouseSuffix;
                quoteContact.BirthDate = form.BirthDate;
        ...
        #region Quote UC
        public int CreateUC(CorviasModel entity)
        {
            Stopwatch sw = new Stopwatch();
            sw.Start();
            using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
            {
                ...
                using (var transaction = db.BeginTransaction())
                {
                    try
                    {
                        var sql = @"insert into dbo.[QuoteUC] ([QuoteKey], [CoverageType], [CoverageMonth], [CoverageYear], [AdditionalQuestions], [PaymentMethod], [AgentCallDate], [AgentCallTime], [NameOnCard], [CardNumber], [ExpiryDate], [SecurityCode], [Routing], [AccountNumber])

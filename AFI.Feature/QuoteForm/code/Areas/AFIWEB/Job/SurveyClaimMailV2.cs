...
{
    public class SurveyClaimMailV2
    {
        ...
        public void Execute(Item[] items, Sitecore.Tasks.CommandItem command, Sitecore.Tasks.ScheduleItem schedule)
        {
            ...
            string receiveTestMail = ConfigurationManager.AppSettings["ReceiveTestMail"];

            List<SurveyForm> mismatchedData = LoadMismatchedData(ConnectionString, AFIConnectionString);

            Sitecore.Diagnostics.Log.Info($"Survay Total Claim : {mismatchedData.Count +" - " +DateTime.Now.ToShortDateString()}", "Mehedi");


            //new added
            Sitecore.Data.Items.Item recEmailtemplate = Sitecore.Context.ContentDatabase.GetItem(ClaimSurveyMail.Email_ItemId);

            string recSubject = " Armed Forces Insurance Claim Survey";
            string recBody = $"Survay Total Claim : {mismatchedData.Count + " - " + DateTime.Now.ToShortDateString()}";
            string recRecipients = recEmailtemplate[ClaimSurveyMail.Recipients];
            string recSender = recEmailtemplate[ClaimSurveyMail.Sender];

            Sitecore.Diagnostics.Log.Info($"Survay Email Send", "Mehedi");

            var _recipient = claimTestMail != null ? claimTestMail : "";

            var recEmail_client = new Email
            {
                IsBodyHtml = true,
                To = new List<string> { _recipient, recRecipients },
                Subject = recSubject,
                Body = recBody,
                FromEmail = recSender
            };

            var isEmailSend = emailService(recEmail_client);

            Sitecore.Diagnostics.Log.Info($"Survay Total Email sent : " + mismatchedData.Count, "Mehedi");

            //if (isEmailSend)
            //{
            //    Sitecore.Diagnostics.Log.Info($"Survay claim insert Receive Email Log.  ", "Mehedi");
            //    _insertEmailLog(recEmail_client, data.ClaimNumber, data.EmailAddress);
            //}
        ...
        private static List<SurveyForm> LoadMismatchedData(string marketingConnectionString, string claimsConnectionString)
        {
            ...
            string marketingQuery = "SELECT * FROM [Survey].[vwClaims]";
            ...
            List<SurveyForm> claimsData = ExecuteQuerySent(claimsConnectionString, claimsQuery);
        ...
        private static List<SurveyForm> ExecuteQuerySent(string connectionString, string query)
        {
        ...
        private static List<SurveyForm> ExecuteQuery(string connectionString, string query)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    SurveyForm surveyForm = new SurveyForm();
                    surveyForm.id = reader["Id"].ToString();
                    surveyForm.Salutation = reader["Salutation"].ToString();
                    surveyForm.FirstName = reader["FirstName"].ToString();
                    surveyForm.LastName = reader["LastName"].ToString();
                    surveyForm.Suffix = reader["Suffix"].ToString();
                    surveyForm.EmailAddress = reader["EmailAddress"].ToString();
                    surveyForm.ClaimNumber = reader["ClaimNumber"].ToString();
                    surveyForm.ClaimClosedDate = reader["ClaimClosedDate"].ToString();
                    surveyForm.CreateDate = reader["CreateDate"].ToString();
    ...
    public class ClaimMail3
    {
        private readonly IEmailService _emailService;

        public ClaimMail3(IEmailService emailService)
        {

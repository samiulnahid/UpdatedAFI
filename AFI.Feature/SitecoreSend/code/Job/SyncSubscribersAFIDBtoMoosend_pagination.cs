...
{
    public class SyncSubscribersAFIDBtoMoosend_pagination
    {
        ...
        public void Execute(Item[] items, Sitecore.Tasks.CommandItem command, Sitecore.Tasks.ScheduleItem schedule)
        {
            try
            {
                ...
                while (true) // Continue until dataList is null
                {
                    dataList = repository.GetFilterSubscriberByIsSynced(false);// Load All IsSynced false data

                    if (dataList == null || dataList.Count == 0)
                        break; // Exit the loop if dataList is null or empty

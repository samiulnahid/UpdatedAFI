...
{
    public interface IQuoteHomenonownerRepository
    {
        IEnumerable<QuoteHomenonowner> GetAll();
        QuoteHomenonowner GetByKey(int key);
        int Create(QuoteHomenonowner entity);
        int CreateOrUpdate(QuoteHomenonowner entity);
    }

    public class QuoteHomenonownerRepository : IQuoteHomenonownerRepository
	{
		private readonly IDbConnectionProvider _dbConnectionProvider;

		public QuoteHomenonownerRepository(IDbConnectionProvider dbConnectionProvider)
		{
			_dbConnectionProvider = dbConnectionProvider;
		}

		public IEnumerable<QuoteHomenonowner> GetAll()
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				var sql = "select * from dbo.[QuoteHomenonowner]";
				return db.Query<QuoteHomenonowner>(sql);
			}
		}
		
		public QuoteHomenonowner GetByKey(int key)
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				var sql = "select top 1 * from dbo.[QuoteHomenonowner] where [Key] = @Key";
				return db.QueryFirstOrDefault<QuoteHomenonowner>(sql, new { key });
			}
		}
		
		public int Create(QuoteHomenonowner entity)
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				db.Open();
				using (var transaction = db.BeginTransaction())
				{
					...
					{
						Log.Error($"{nameof(QuoteHomenonownerRepository)}: Error while attempting to insert QuoteHomenonowner.", ex, this);
        ...
        public int CreateOrUpdate(QuoteHomenonowner entity)
        {
            using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
            {
                db.Open();
                using (var transaction = db.BeginTransaction())
                {
                    try
                    {
                        var sql = @"MERGE [dbo].[QuoteHomenonowner] as target
                    ...
                    {
                        Log.Error($"{nameof(QuoteHomenonownerRepository)}: Error while attempting to update QuoteHomenonowner.", ex, this);
                        transaction.Rollback();
                        return 0;
                    }
                }

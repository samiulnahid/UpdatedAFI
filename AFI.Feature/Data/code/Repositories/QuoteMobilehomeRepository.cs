...
{
    public interface IQuoteMobilehomeRepository
    {
        IEnumerable<QuoteMobilehome> GetAll();
		QuoteMobilehome GetByKey(int key);
        int Create(QuoteMobilehome entity);
        int CreateOrUpdate(QuoteMobilehome entity);
    }

    public class QuoteMobilehomeRepository : IQuoteMobilehomeRepository
	{
		private readonly IDbConnectionProvider _dbConnectionProvider;

		public QuoteMobilehomeRepository(IDbConnectionProvider dbConnectionProvider)
		{
			_dbConnectionProvider = dbConnectionProvider;
		}

		public IEnumerable<QuoteMobilehome> GetAll()
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				var sql = "select * from dbo.[QuoteMobilehome]";
				return db.Query<QuoteMobilehome>(sql);
			}
		}
		
		public QuoteMobilehome GetByKey(int key)
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				var sql = "select top 1 * from dbo.[QuoteMobilehome] where [Key] = @Key";
				return db.QueryFirstOrDefault<QuoteMobilehome>(sql, new { key });
			}
		}
		
		public int Create(QuoteMobilehome entity)
		{
			using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
			{
				db.Open();
				using (var transaction = db.BeginTransaction())
				{
					...
					{
						Log.Error($"{nameof(QuoteMobilehomeRepository)}: Error while attempting to insert QuoteMobilehome.", ex, this);
        ...
        public int CreateOrUpdate(QuoteMobilehome entity)
        {
            using (var db = _dbConnectionProvider.GetAFIDatabaseConnection())
            {
                db.Open();
                using (var transaction = db.BeginTransaction())
                {
                    try
                    {
                        var sql = @"MERGE [dbo].[QuoteMobilehome] as target
                    ...
                    {
                        Log.Error($"{nameof(QuoteMobilehomeRepository)}: Error while attempting to update QuoteMobilehome.", ex, this);
                        transaction.Rollback();
                        return 0;
                    }
                }
